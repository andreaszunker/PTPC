...
unsigned long enumerate_subtree(
    ...
    memcpy(message, start_message, sizeof(int64_t[message_size]));
...
struct enumeration_result enumerate_minimum_weight_codewords(size_t K, size_t N, uint8_t generator_matrix[K][N]) {
    ...
    size_t message_size = ((N-1) >> 6)+1; // the message is stored bitwise -> faster checking of the dynamic frozen bits
